...
def main():
    ...
    labels = pd.read_csv('../HGDP/HGDPpop.csv', index_col=0).values.ravel()
    HGDPdata = pd.read_csv('../HGDP/HGDPdata.csv', index_col=0).values
    print('labels array:', labels.dtype, labels.shape)
    print('HGDPdata array:', HGDPdata.dtype, HGDPdata.shape)
    n_clusters = 79
    Npc = 1000
    max_iter = 500

    test(HGDPdata, labels, n_clusters, Npc, max_iter, 'HGDPdata')
