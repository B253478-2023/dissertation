...
def main():
    # Load the genotype matrix with labels
    genotype_df = pd.read_csv('genotype_matrix_with_labels.csv')

    # Convert the genotype data to a NumPy array
    genotype_matrix = genotype_df.values  # or genotype_df.to_numpy()

    # Alternatively, if labels were saved separately:
    pop_labels_df = pd.read_csv('pop_labels.csv')
    pop_labels = pop_labels_df['pop_label'].values.ravel()

    # Print the shape of the matrix and some labels as a sanity check
    print("Genotype Matrix Shape:", genotype_matrix.shape)
    print(genotype_matrix.dtype)
